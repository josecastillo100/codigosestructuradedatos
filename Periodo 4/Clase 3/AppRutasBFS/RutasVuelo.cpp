//Created by Jose Alfredo Castillo on 04/12/2021
...
#include "Utilidades.h"		//Referencia a clase utilidades
...
void procesarArchivoCiudades (string nombreArchivo) {
	...
	while (getline (archivo, ciudad)) {
		...
		cout << "Se agrego la ciudad " << ciudad << " - (Arista " <<(ciudades.size() - 1) << ")" << endl;
	}
...
void procesarSolicitudVuelo()
{
	...
	{
		_Utilidad.AplicarColorDinamico_ConTexto(FOREGROUND_INTENSITY | FOREGROUND_RED, "Lo siento, no volamos hacia esa ciudad.", true);
	...
	if (!itinerario.existeCaminoHacia(codigosCiudades[ciudadDestino])) {
		...
		for (it = conexiones.begin(); it != conexiones.end(); it++) {
			...
			if (codigo == codigosCiudades[ciudadDestino])
			{
				...
				_Utilidad.AplicarColorDinamico_ConTexto(FOREGROUND_INTENSITY | FOREGROUND_BLUE, ciudadDestino, true);
			}	
			else
			{
				_Utilidad.AplicarColorDinamico_ConTexto(FOREGROUND_INTENSITY | FOREGROUND_BLUE, ciudades[codigo] + " -> ", false);
...
int main() 
{
	SetConsoleTitleA("Ejercicios de Rutas de Vuelo con BFS y Grafos - Tarea UFG"); //Cambiar el titulo de la consola
